...
if [ ! -f /etc/hiawatha/tls/selfcertwithkey.pem ]; then
  ...
  /usr/bin/gen_key type=rsa rsa_keysize=4096 filename=privkey.pem && \
  /usr/bin/cert_write selfsign=1 issuer_key=privkey.pem issuer_name=CN=myserver,O=myorganisation,C=US is_ca=1 max_pathlen=0 output_file=selfcert.pem && \
